...
## Medium-term source reliability

medium-term-v2 requires source dates, DART comparable-period fundamentals, positive equity and at least 60% weighted data coverage. Missing data receives no score and coverage is not probability. The UI exposes evidence coverage and DART growth; all candidates remain research-only. See PredictionReliability.md for source contracts and additional API requirements.
